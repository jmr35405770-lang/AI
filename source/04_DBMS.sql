...
정수 : tinyint(ibyte), smallint(2byte), dediumint(3byte)
...
select * person where sal between 2000 and 5000;
select * person where sal>=2000 && sal<=5000;
select * person where sal>=2000 and sal<=5000;
...
-- 7. 사번, 이름, 상사이름
select emp.pno, emp.pname, mgr.pname as manager_name
from person emp
join person mgr on emp.manager = mgr.pno;
-- 8. 사번, 이름, 상사이름(상사가 없는 사람도 출력하되 상사가 없는 경우 ★CEO★로 출력) – oracle과 다른 문법
select 
    emp.pno, 
    emp.pname, 
    ifnull(mgr.pname, '★CEO★') as manager_name
from person emp
left join person mgr on emp.manager = mgr.pno;
-- 8-1 사번, 이름, 상사사번(상사가 없으면 ceo로 출력. ifnull함수의 매개변수의 타입이 상이해도 상관없음) – oracle과 다른 문법
select pno, pname, ifnull(manager, 'CEO') as manager_pno
from person;
...
select 
    ...
    p.sal, 
    d.dname, 
    ifnull(m.pname, '★CEO★') as manager_name
from person p
join division d on p.dno = d.dno
left join person m on p.manager = m.pno;
-- oracle과 다른 함수들
